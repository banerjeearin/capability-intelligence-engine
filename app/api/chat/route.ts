...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const evidenceText = retrieval.results.map((item, idx) => `[Evidence ${idx + 1}] ${item.content}`).join('\n\n');
    ...
    if (retrieval.confidence < 0.35 || !retrieval.results.length) {
      ...
      await appendConversationMessage(conversationId, userId, 'assistant', guarded.answer);
      structuredLog('chat_metrics', {
        traceId: trace.traceId,
        confidence: retrieval.confidence,
        citations: citations.length,
        lowConfidence: true,
        guardrail: guarded.reason
      });
    ...
    if (!aiRes.ok) {
      ...
      endSpan(trace, { success: false, reason: 'openai_failure' });
    ...
    endSpan(trace, { success: true, guardrail: guarded.reason });
    ...
    endSpan(trace, { success: false, reason: 'exception' });
    return NextResponse.json(
